...
import { ShieldAlert, Lock, User, Mail, ShieldCheck, ArrowRight, AlertCircle } from 'lucide-react';

interface SignupViewProps {
  onSignupSuccess: (user: { username: string; role: string; token: string }) => void;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem('vigil_user', JSON.stringify({ username: data.username, role: data.role }));
        onSignupSuccess({ username: data.username, role: data.role, token: data.access_token });
      } else {
        const errData = await res.json().catch(() => ({ detail: 'Registration failed' }));
        setError(errData.detail || 'Could not create account. Username or Email may already exist.');
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#0B0F17] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#121824] border border-[#26334D] rounded-2xl p-8 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold text-white tracking-wide">Create VIGIL Account</h1>
          <p className="text-xs text-blue-400 uppercase tracking-widest font-mono">Sign Up For Voice Defense Access</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            <label className="block text-gray-400 mb-1.5 font-medium">Password</label>
            <div className="relative">
          ...
          <div>
            <label className="block text-gray-400 mb-1.5 font-medium">Security Role</label>
            <select
              value={role}
            ...
            >
              ...
              <option value="OPERATOR">Live Operations Monitor</option>
