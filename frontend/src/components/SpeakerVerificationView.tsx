...
export const SpeakerVerificationView: React.FC = () => {
  ...
  const fetchProfiles = async () => {
    try {
      const res = await fetch('http://://vigil-backend-bbwj.onrender.com/api/speaker/profiles');
      if (res.ok) {
  ...
  const handleEnroll = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('http://://vigil-backend-bbwj.onrender.com/api/speaker/register', {
        method: 'POST',
        body: formData
      });
