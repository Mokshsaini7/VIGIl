...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess, onSwitchToSignup }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('http://https://vigil-backend-bbwj.onrender.com/api/auth/login', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-[#0B0F17] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#121824] border border-[#26334D] rounded-2xl p-8 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold text-white tracking-wide">VIGIL</h1>
          <p className="text-xs text-blue-400 uppercase tracking-widest font-mono">Voice Security Operations Center</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          >
            <span>{loading ? 'Authenticating...' : 'Sign In to VIGIL'}</span>
